...
// Secondary client for creating users without affecting the admin's session.
// persistSession: false ensures signUp() never writes to localStorage,
// so the main client's onAuthStateChange is never triggered.
export const supabaseAdmin = createClient(supabaseUrl, supabaseAnonKey, {
  auth: {
    autoRefreshToken: false,
    persistSession: false,
    storageKey: 'supabase-admin-session', // isolated from the main client
  },
});
